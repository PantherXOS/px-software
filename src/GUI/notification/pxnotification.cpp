...
#include "pxnotification.h"
#include "pxnotification_p.h"
#include <QMessageBox>
#include <QDebug>
...
{
    Q_D(Notification);
    d->mHints.insert(QLatin1String("urgency"), qvariant_cast<uchar>(urgency));
}
...
NotificationPrivate::NotificationPrivate(const QString& summary, Notification* parent) :
    ...
    q_ptr(parent)
{
    mInterface = new OrgFreedesktopNotificationsInterface(QLatin1String("org.freedesktop.Notifications"),
...
void NotificationPrivate::update()
{
    ...
    {
        if (mHints.contains(QLatin1String("urgency")) && mHints.value(QLatin1String("urgency")).toInt() != Notification::UrgencyLow)
            QMessageBox::information(nullptr, tr("Notifications Fallback"), mSummary + QLatin1String(" \n\n ") + mBody);
    }
}
...
void NotificationPrivate::setActions(QStringList actions, int defaultAction)
{
    ...
    {
        if (ix == defaultAction)
            mActions.append(QLatin1String("default"));
        else
